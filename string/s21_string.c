...
char* s21_strtok(char* str, const char* delim) {
    ...
    if (str != s21_NULL) {
        for (int i = 0; delim[i] != '\0'; i++) {
            char* find = s21_strchr(str, delim[i]);
            if (find != s21_NULL) {
                if (s21_strlen(find) == s21_strlen(str)) {
                    str++;
                    i = -1;
                    buf = s21_NULL;
                    continue;
                }
                if ((buf == s21_NULL) || (s21_strlen(find) > s21_strlen(buf))) {
                    buf = find;
                }
            }
        }
...
char* s21_strcat(char* dest, const char* src) {
    int len1 = s21_strlen(dest), len2 = s21_strlen(src);
...
char* s21_strrchr(char* str, int ch) {
    int i = 0, n = 0;
    while (str[i]) {
        if (str[i] == ch)
            n = i;
...
char* s21_strerror(int errnum) {
    ...
    if (errnum >= 0 && errnum < 134)
        s21_memmove(msg, (char*)lin[errnum], s21_strlen((char*)lin[errnum]) + 1);
    else
        s21_memmove(msg, s21_strcat(u, s), s21_strlen(u) + s21_strlen(s) + 1);
    answer = msg;
    ...
    if (errnum >= 0 && errnum < 107)
        s21_memmove(msg, (char*)Mac[errnum], s21_strlen((char*)Mac[errnum]) + 1);
    else
        s21_memmove(msg, s21_strcat(u, s), s21_strlen(u) + s21_strlen(s) + 1);
    answer = msg;
#endif
...
char* s21_strncat(char* dest, const char* src, s21_size_t n) {
    ...
    for (s21_size_t i = len1; i < len1 + n; i++, src++)
        dest[i] = *src;
    dest[len1 + n] = '\0';
    return dest;
}
...
int s21_strncmp(const char* str1, const char* str2, s21_size_t n) {
    int out = 0;
    s21_size_t i = 0;
    while (*str2 && *str1 && i < n && (str1[i] == str2[i]))
        i++;
    if (i < n && n > 0)
        out = str1[i] - str2[i];
    return out;
...
void width_s_or_c(char* str, int len_string, int* len, parsing pars) {
    ...
    if (pars.width != 0 && !pars.flag_minus) {
        if (pars.precision_number > 0 && pars.precision_number < len_string) {
            prec = pars.precision_number;
        }
        if (!prec)
            prec = len_string;

        for (int w = pars.width - prec; w > 0; w--)
            str[(*len)++] = ' ';
    }
}

void precision(char* str, int i, int* len, parsing pars) {
    for (int w = pars.precision_number - i; w > 0; w--, (*len)++)
        str[*len] = '0';
}
...
void width_minus(char* str, int i, int* len, parsing pars) {
    ...
    if (pars.precision_number > i)
        ...
        main_widht = i;
    if (pars.flag_minus) {
        for (int w = pars.width - main_widht; w > 0; w--)
            str[(*len)++] = ' ';
    }
}
...
void width_minus_d(char* str, int i, int* len, parsing pars, int d) {
    int main_widht = 0;
    int check = 0;
    if (d < 0 || pars.flag_plus == 1 || pars.flag_space == 1)
        check = 1;
    if (pars.precision_number > i)
...
void width_f(char* str, int f, int* len, parsing pars) {
    int check = 0;
    int d = f;
...
void String_to_char(char* str, char* string, int* len, parsing pars) {
    width_s_or_c(str, s21_strlen(string), len, pars);
    str[*len] = '\0';
    if (pars.precision_number >= 0)
        s21_strncat(str, string, pars.precision_number);
    else
        s21_strcat(str, string);
    *len = s21_strlen(str);
    width_minus(str, s21_strlen(string), len, pars);
}
...
void translate_unsigned_long_int_to_char(unsigned long int d, char* r, int* i, int precision_var) {
    if (d == 0) {
...
void Unsigned_int_to_char(char* str, unsigned long d, int* len, parsing pars) {
    if (pars.length_h == 1)
        ...
        d = (unsigned int)d;
    if (pars.width != 0 && !pars.flag_minus)
        width_u(str, d, len, pars);
    char r[1000];
    int i = 0;
    translate_unsigned_long_int_to_char(d, r, &i, 1);
    r[i] = '\0';
    if (pars.precision_number >= 0)
        precision(str, i, len, pars);
    if (d == 0 && pars.precision_number == 0) {
        i--;
    } else {
...
void float_to_char(char* str, double f, int* len, parsing pars) {
    ...
    if (precision_var == 0 && d >= 0.5)  // округление до целого
        integ_part++;
    ...
    if (pars.width != 0 && !pars.flag_minus)  // отступ слева
        width_f(str, integ_part * sign, len, pars);
    if (sign == -1) {
    ...
    if (precision_var != 0) {
        ...
        translate_unsigned_long_int_to_char(d1, r, &len_num, precision_var);  // перевод дробной части в чары
        ...
        if (len1 >= start_index) {
            ...
            if (newstr != s21_NULL) {
                for (s21_size_t i = 0; i < start_index; i++)
                    newstr[i] = src[i];
                for (s21_size_t i = start_index; i < start_index + len2; i++)
                    newstr[i] = str[i - start_index];
                for (s21_size_t i = start_index + len2; i < len1 + len2; i++)
                    newstr[i] = src[i - len2];
                newstr[len1 + len2] = '\0';
                p = newstr;
            }
...
void* s21_trim(const char* src, const char* trim_chars) {
    ...
    if (src != s21_NULL && trim_chars != s21_NULL) {
        ...
        if (newstr != s21_NULL) {
            for (int i = left, j = 0; i < len1 - right; i++, j++)
                newstr[j] = src[i];
